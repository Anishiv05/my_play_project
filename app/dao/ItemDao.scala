...
(implicit executionContext: ExecutionContext)
  ...
  def all(relatedCategories: Seq[Int]): Future[Seq[Item]] = {
    val query = Items.filter(_.category_id.inSet(relatedCategories.toSet))
    db.run(query.result).map((_.sortBy(_.title)))
