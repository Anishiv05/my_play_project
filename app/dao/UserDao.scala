...
trait UserTrait { self: HasDatabaseConfigProvider[JdbcProfile] =>
  ...
  class UserTable(tag: Tag) extends Table[User](tag, "USER") { // USER - table name in db
...
(implicit executionContext: ExecutionContext)
  ...
  private val Users = TableQuery[UserTable] // Users -  a TableQuery object representing the database table where users are stored.
